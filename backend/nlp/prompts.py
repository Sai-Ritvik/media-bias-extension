...
<rules>
1. You must extract exactly 4 to 6 highly relevant keywords or 2-word key-phrases (lowercase). Focus on the core subject matter (e.g., "tax policy", "agricultural reform"). Ignore generic words like "news".
2. You must classify the bias strictly as "left", "right", or "center".
3. Your explanation must be exactly 1 to 2 sentences.
4. Your explanation MUST quote 1 or 2 specific emotionally charged phrases directly from the text that justify the bias. 
5. If the bias is "center", state that the text relies on factual reporting, but DO NOT invent or hallucinate quotes that are not in the text.
6. You must output ONLY valid JSON. Do not include any conversational text.
</rules>
